...
!  avoid casts which are relatively expensive
       ...
       if( isym == 3 ) then
          ...
          do m = 1, mmax
             do l = philwb, phiupb
                bes_cos(l,m) = cos(0.5*dphi*(mindex-1.0)*    &
                                        (2.0*lindex-1.0))
                bes_sin(l,m) = sin(0.5*dphi*(mindex-1.0)*    &
                                        (2.0*lindex-1.0))
                lindex = lindex + 1.0
             enddo
             lindex = 1.0
...
!  potential - tmr and the Green function for the side potential - smz
      call tm(tmr)          !wth?
      call sm(smz)
!      open(unit=10,file='/scratch1/phmotl/tmr',form='unformatted',  &
!          status='unknown')
!      read(10) tmr
!      close(10)
!      open(unit=11,file='/scratch1/phmotl/smz',form='unformatted',  &
!          status='unknown')
!      read(11) smz
!      close(11)
       ...
       if( isym == 3 ) then
          do l = philwb, phiupb
             elambdazb(2,l) = alphaz(2) + 2.0*betaz(2) + 2.0*     &
                             (elm(l)-1.0)*dphiinv2*rhfinv(2)*rhfinv(2)
          enddo
       endif
       if( isym /= 3 ) then
          do l = philwb, phiupb
             elambdazb(2,l) = alphaz(2) - (m1mode(l)-1.0)*betaz(2) +      &
                             2.0*(elm(l)-1.0)*dphiinv2*rhfinv(2)*         &
                             rhfinv(2)
          enddo
       endif
!       open(unit=19,file='elambdazb',form='unformatted',
!     &      status='unknown')
!       write(19) elambdazb
!       close(19) 
