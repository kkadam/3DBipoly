...
!*  Local Variables
       ...
       real, dimension(numr,numphi) :: phibTMP, potb, phibTMPC,    &
                                         phibTMPS
...
!  of the potential
       ...
       if( isym == 1 ) then
          do j = 2, rupb
             StC(j,1) = sum(tmr(rlwb:rupb,zlwb:zupb,j,1)*       &
                           TMPC(rlwb:rupb,zlwb:zupb))
             SbC(j,1) = sum(tmr(rlwb:rupb,zupb:zlwb:-1,j,1)*    &
                           TMPC(rlwb:rupb,zlwb:zupb))
          enddo
          do k = 2, zupb 
             SsC(k,1) = sum(smz(rlwb:rupb,zlwb:zupb,k,1)*       &
                           TMPC(rlwb:rupb,zlwb:zupb))
          enddo
       else
          do j = 2, rupb
...
!  modes with m > 0
       if( isym == 1 ) then
          do m = 2, mmax
             ...
             enddo
             do j = 2, rupb
                StC(j,m) = sum(tmr(rlwb:rupb,zlwb:zupb,j,m)*          & 
                              TMPC(rlwb:rupb,zlwb:zupb)) 
                StS(j,m) = sum(tmr(rlwb:rupb,zlwb:zupb,j,m)*          &
                              TMPS(rlwb:rupb,zlwb:zupb))
                SbC(j,m) = sum(tmr(rlwb:rupb,zupb:zlwb:-1,j,m)*       &
                              TMPC(rlwb:rupb,zlwb:zupb))
                SbS(j,m) = sum(tmr(rlwb:rupb,zupb:zlwb:-1,j,m)*       &
                              TMPS(rlwb:rupb,zlwb:zupb))
             enddo
             do k = 2, zupb
                SsC(k,m) = sum(smz(rlwb:rupb,zlwb:zupb,k,m)*          &
                              TMPC(rlwb:rupb,zlwb:zupb))
                SsS(k,m) = sum(smz(rlwb:rupb,zlwb:zupb,k,m)*          &
                              TMPS(rlwb:rupb,zlwb:zupb))
             enddo
          enddo
       else
